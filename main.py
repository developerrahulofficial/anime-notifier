...
class aclient(commands.Bot):    # Inherits from commands.Bot
	...
	async def on_ready(self):
		...
		await self.change_presence(activity=disnake.Activity(
			type=disnake.ActivityType.watching, name="DeveloperRahul"))
...
def animeListEmbedCard(anime_series):
	...
	embed.set_footer(text="by Developer Rahul")
				...
				if anime.lower() in episode_text:
					...
					if str(episode_number_db) != str(episode_number):
						...
						if not airing:
							...
							await channel.send(f"Anime '{anime}' is no longer airing at episode '{episode_number}'")
...
async def notify(anime_name, anime_id,episode_number, airing=True):
    ...
    await  channel.send(message,embed=episodeEmbedCard(anime_id,episode_number,airing=True)
                        )
